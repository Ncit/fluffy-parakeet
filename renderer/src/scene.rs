...
fn validate_scene(version: u32, duration: f32, fps: f32, width: u32, height: u32, nodes: &[SceneNode]) -> Result<(), SceneLoadError> { let mut errors = Vec::new(); if version != SUPPORTED_DSL_VERSION { errors.push(format!("unsupported DSL version {version}; expected {SUPPORTED_DSL_VERSION}")); } if duration <= 0.0 { errors.push("duration must be greater than 0".to_string()); } if fps <= 0.0 { errors.push("fps must be greater than 0".to_string()); } if width == 0 || height == 0 { errors.push("project width and height must be positive".to_string()); } let mut ids = HashSet::new(); for (index, node) in nodes.iter().enumerate() { if node.id.trim().is_empty() { errors.push(format!("node {index} is missing id")); } if node.id.chars().any(char::is_whitespace) { errors.push(format!("node '{}' id cannot contain whitespace", node.id)); } if !matches!(node.kind.as_str(), "rect" | "text" | "image") { errors.push(format!("node '{}' has unsupported type '{}'", node.id, node.kind)); } if node.width <= 0.0 || node.height <= 0.0 { errors.push(format!("node '{}' width and height must be positive", node.id)); } if let Some(end_time) = node.end_time { if end_time < node.start_time { errors.push(format!("node '{}' end_time must be >= start_time", node.id)); } } if !ids.insert(node.id.clone()) { errors.push(format!("duplicate node id '{}'", node.id)); } } for node in nodes { if let Some(parent_id) = &node.parent_id { if !ids.contains(parent_id) { errors.push(format!("node '{}' references missing parent_id '{}'", node.id, parent_id)); } if parent_id == &node.id { errors.push(format!("node '{}' cannot parent itself", node.id)); } } } if errors.is_empty() { Ok(()) } else { Err(SceneLoadError::Validation(errors)) } }

#[cfg(test)]
mod tests { use super::*; #[test] fn applies_style_and_project_metadata() { let scene = Scene::from_dsl_json(r#"{"version":3,"duration":1,"fps":24,"width":1280,"height":720,"background":[0,0,0,1],"styles":{"card":{"color":[1,0,0,1],"width":2}},"nodes":[{"id":"n1","type":"rect","style":"card"}]}"#).unwrap(); assert_eq!(scene.fps, 24.0); assert_eq!(scene.width, 1280); assert_eq!(scene.nodes[0].color, [1.0,0.0,0.0,1.0]); assert_eq!(scene.nodes[0].width, 2.0); } #[test] fn validates_asset_references() { let scene = Scene::from_dsl_json(r#"{"version":3,"duration":1,"assets":{"logo":{"type":"image","src":"assets/logo.png"}},"nodes":[{"id":"img","type":"image","asset":"logo"}]}"#).unwrap(); assert_eq!(scene.asset_by_id("logo").unwrap().src, "assets/logo.png"); } }