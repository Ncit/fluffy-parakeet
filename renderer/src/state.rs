...
impl<'window> State<'window> {
    pub async fn new(window: &'window Window) -> Self {
        ...
        let render_objects = scene.nodes.iter().enumerate().map(|(index, _node)| {
            let uniform = scene.sample_node(index, 0.0).uniform;
        ...
        Self { surface, device, queue, config, size, pipeline, shader, vertex_buffer, scene, render_objects, time: 0.0 }
    }
    ...
    pub fn render(&mut self) {
        ...
        for (index, (node, render_object)) in self.scene.nodes.iter().zip(self.render_objects.iter()).enumerate() {
            if !node.is_active(timeline_time) { continue; }
            let uniform: NodeUniform = self.scene.sample_node(index, timeline_time).uniform;
            self.queue.write_buffer(&render_object.uniform_buffer, 0, bytemuck::bytes_of(&uniform));
        }
        let frame = match self.surface.get_current_texture() {
            Ok(frame) => frame,
            Err(wgpu::SurfaceError::Lost | wgpu::SurfaceError::Outdated) => { self.resize(self.size); return; }
